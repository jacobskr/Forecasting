  ...
  server = function(input, output) 
  {
      ...
      if (input$FC_Model == "Naive") {
        ...
        shiny::withMathJax(
          helpText('ETS(A,N,N) Model: $$Measurement: y_t = \\ell_{t-1} + \\varepsilon_t$$',
                   ...
                   '$$State Equation: \\varepsilon_{t-1} = 8754 + .4107\\varepsilon_t$$'))
      }
      else if (input$FC_Model == "STLM") {
        shiny::withMathJax(
          helpText(''))
    ...
    output$model = renderImage({
      ...
      filename <- normalizePath(file.path(getwd(),
                                paste('jpeg/', input$FC_Model, '.jpg', sep='')))
    ...
    output$final = renderPlotly({
      if (input$val) {
       bval <- autoplot(oil_freq.tr) + autolayer(comb_best) +
       ...
       ggplotly(bval)
        ...
        fc_stlm_for <- forecast(fit_stlm_for, h=input$weeks)
        
        fit_base_for <- Arima(oil_freq, model = fit_base)
        fc_base_for <- forecast(fit_base_for, h=input$weeks)
        ...
        fc_fper_for <- forecast(fit_fper_for, h=input$weeks,
                                xreg=cbind(
                                  fourier(ts(oil_freq, frequency=54), K=3, h=input$weeks),
                                  fourier(ts(oil_freq, frequency=72), K=1, h=input$weeks),
                                  fourier(ts(oil_freq, frequency=30.85714286), K=3, h=input$weeks)))
        
        fit_seasons_for <- Arima(oil_freq, xreg = seasons, model = fit_seasons)
        fc_seasons_for <- forecast(fit_seasons_for, xreg=seasonsfc, h=input$weeks)
        
        fit_arima.msts_for <- Arima(oil_msts[, "barrels"], xreg=oil_msts[, c(2,3,4)], model = fit_arima.msts)
        fc_arima.msts_for <- forecast(fit_arima.msts_for, xreg=seasonsfc, h=input$weeks)
        
        fit_nn.xregs_for <- nnetar(oil_freq, xreg = seasons)
        fc_nn.xregs_for <- forecast(fit_nn.xregs_for, xreg = seasonsfc, PI=T, h=input$weeks)
        ...
        bfor <- autoplot(oil_freq) + autolayer(comb_best_for) +
